...
const CPKChart = ({ data, cpkList, serverId, startDate, endDate, selectedProject }) => {
  ...
  // if (!selectedProject) {
  //   console.error('CPKChart: project is required but not provided');
  //   return <div>Error: Project is required</div>;
  // }
  ...
  const handleBarClick = async function () {
    ...
    const requestData = {
      serverId: serverId,        // Ensure this is passed correctly
      projCode: selectedProject,         // Use projCode instead of project
      startDate: startDate,
      endDate: endDate,
      lineNo: null,
      paramName: paramName,
    ...
    console.log('CPK Scatter Request Data:', requestData); // Debug log

    const apiPayload = createCPKData(requestData, paramName);
    console.log('CPK API Payload:', apiPayload); // Debug log

    try {
      const scatterResponse = await apiService.getScatteredData(apiPayload);
      console.log('Scatter Response:', scatterResponse); // Debug log
  ...
  const scatterOptions = {
    ...
    tooltip: {
      formatter: function () {
        return `<b>Sample ${this.point.x}</b><br/>Value: ${this.point.y}`;
      },
    },
    series: [
      {
