...
// Add these imports
import { apiService, createCPKData, createProjectData } from '../services/apiService';
...
const Dashboard = () => {
  ...
  const ServerInfo = [
    ...
    // ✅ NEW: Load CPK parameters from localStorage on mount
  useEffect(() => {
...
const GetFPYDatas = async (option = 3) => {
  try {
    ...
    const projectData = {
      ...
      stage: 1,  // ✅ Set to 0 to get all stages (based on your API logic)
...
const handleGetFPYData = async () => {
  ...
  const GetCPKData = async () => {
    try {
      ...
      }, null, 1); // Option 1 for CPK values
    //    debugger
  ...
  //  debugger
      if (data && Array.isArray(data) && data.length > 0) {
        ...
        // ✅ Create CPK list based on configured parameters
        const cpkListData = data.map((item, index) => {
          // Check if this parameter is enabled in configuration
                ...
                {cpkData && cpkData.length > 0 && (
                  <div className="chart-block">
                    <CPKChart
                      ...
                      endDate={formData.endDate}
                      projCode={formData.project}
                      cpkParameters={cpkParameters} // ✅ Pass CPK parameters
                    />
                  </div>
                )}
              </div>
